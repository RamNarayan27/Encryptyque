...
const rec_list = require('data-store')({ path: process.cwd() + '\\friend_list.json' })
const out_list = require('data-store')({ path: process.cwd() + '\\outbox.json' })
...
async function secure_session(job, payloaddata) {
    ...
    server_pubkey = NodeRSA(server_public_key, 'pkcs8-public-pem')
    ...
    json_payload = stringify(payload)
    encoded_payload = Base64.encode(json_data)
    encrypted_payload = pubkey.encrypt(encoded_data)

    payload_promise = new Promise(function (resolve, reject) {
        ws.send(encrypted_payload);
        ws.on('message', function incoming(data) {
            if (data !== 'FAIL') {
                if (data == 'DONE') {
                    ...
                    decrypted_data = server_pubkey.decryptPublic(data);
                    decrypted_data = decrypted_data.toString()
                    decoded_data = Base64.decode(decrypted_data)
                    final_data = parse(decoded_data)
                    resolve(final_data)
                }
            }
        });
...
conf.set('IMPORTANT!!!!', 'DO NOT SHARE THIS WITH ANYONE'); conf.set('', '');
if (!conf.has('unique-username')) { const temp_username = randomstring.generate(5); conf.set('unique-username', temp_username) }
...
function mypublickey() { return parse(conf.get('publickey')) }
function myprivatekey() { return parse(conf.get('privatekey')) }
...
async function file_prepare(file, recipient_list) {
    let promise = new Promise(function (resolve, reject) {
        ...
        encoded_promise.then(function (result) {
            key = crypto.pbkdf2Sync(randomstring.generate(10), crypto.randomBytes(16), 100000, 256 / 8, 'sha256')
            ...
            function (result) {
                ...
                function (result) {
                    file_manager.writeFile(process.cwd() + '\\outbox\\' + result[5], result[0],
                        function (err) {
                            if (err) throw err;
...
async function download_file(downloadID) {
    ...
    s3.getObject(params, function (err, data) {
        if (err) {
            ...
            file_manager.writeFileSync(process.cwd() + '\\inbox\\' + ef_actual_name, ua)
        }
    })
}
...
async function delete_file(downloadID) {
    ...
    try {
        file_manager.access(path, file_manager.F_OK, (err) => {
            ...
            file_manager.unlink(path, callback = function () {
...
//send_file()
//refresh_check()
//download_file('D5Fl5cm8jEqm')
//delete_file('1ICx9aAEYVCc')
//setinboxlength()